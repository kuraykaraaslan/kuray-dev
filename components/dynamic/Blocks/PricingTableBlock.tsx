        ...
        <div className="grid md:grid-cols-3 gap-8 items-end">
          {plans.map((plan, i) => (
            ...
            >
              ...
              {plan.description && (
                <p className="text-sm mb-6" style={{ color: 'oklch(var(--bc) / 0.6)' }}>
                  {plan.description}
                </p>
              )}
              ...
              <div className="mb-6">
                ...
                {plan.period && (
                  <span className="text-sm" style={{ color: 'oklch(var(--bc) / 0.6)' }}>
                    /{plan.period}
                  </span>
                )}
              </div>
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map((feature, fi) => (
                  <li key={fi} className="flex items-start gap-3">
                    ...
                    <span style={{ color: 'oklch(var(--bc) / 0.8)' }}>{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              {plan.cta && plan.ctaHref && (
                <Link
                  href={plan.ctaHref}
                  className={`block text-center py-3 rounded-md font-medium transition-transform hover:scale-105{!accent ? ' bg-primary text-primary-content' : ''}`} style={accent ? { backgroundColor: accent, color: 'oklch(var(--pc))' } : undefined}
                >
                  {plan.cta}
                </Link>
              )}
